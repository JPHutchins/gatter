...
def test_stream_id_type():
    s = StreamId(device_id=4, stream_id=8)
    assert s
    assert s.device_id == 4
    assert s.stream_id == 8

    with pytest.raises(ValueError):
        s = StreamId(device_id=4)

    with pytest.raises(ValueError):
        s = StreamId(stream_id=4)

    with pytest.raises(ValueError):
        s = StreamId(device_id=-1)

    with pytest.raises(ValueError):
        s = StreamId(stream_id=256)

    with pytest.raises(ValueError):
        s = StreamId()

    s = StreamId(device_id=5, stream_id=12)
...
def test_stream_manager_adds_streams():
    s0 = StreamId(device_id=0, stream_id=0)
    s1 = StreamId(device_id=1, stream_id=0)
...
def test_stream_manager_removes_streams():
    s0 = StreamId(device_id=0, stream_id=0)
    s1 = StreamId(device_id=1, stream_id=0)
